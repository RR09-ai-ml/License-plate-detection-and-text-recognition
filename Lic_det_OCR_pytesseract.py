...
def Camera_capture():
    ...
    # taking image.jpg
    image_path = "car2.jpg"  # paste your image path
    image = cv2.imread(image_path)

    # taking image from camera
    # image = cv2.VideoCapture(0)
    # result, image = image.read()
    ...
    image = cv2.rectangle(resize_image, (x,y), (x+w,y+h), (0,0,255), 3) 
...
# To Create a tkinter (GUI) window
window = tk.Tk()
window.title("Detection Menu")

label = tk.Label(window, text = "Car Detection System", font="Algerian", underline = True , width = 20, height = 5)
label.pack()

button1 = tk.Button(window, text="Camera capture", command=Camera_capture)
button1.pack(pady=10)  # Add some padding around the button
